...
当前是 **0.5.39 测试版**。Mac 版尚未完成 Apple Developer ID 签名与公证，也未上架 Mac App Store；Android 尚未上架 Google Play。Intel Mac 和 iOS 版本暂未提供。
...
Mac 安装版菜单栏新增“检查 Mac 更新…”与自动检查开关。下载、签名校验、安装和重启由 Sparkle 完成；配对和设置保留，自动安装默认关闭。**0.5.37 或更早版本需先手动安装一次 0.5.38，此后即可在菜单栏更新。** 当前仍为未完成 Apple 公证的测试版。详见 [后台验证范围](https://github.com/yx3110/remote-vibe/releases/download/v0.5.38/verification-0.5.38.md)。

## 0.5.39 解锁进度与故障提示

指纹验证前检查 Mac 权限，解锁请求不再等待手机的普通控制队列，分别显示发送与确认状态，失败原因保留显示。继续兼容旧版 Mac，不会自动重发密码。另一台实体 Mac 的原始故障仍待实际验证，详见 [验证范围](https://github.com/yx3110/remote-vibe/releases/download/v0.5.39/verification-0.5.39.md)。
