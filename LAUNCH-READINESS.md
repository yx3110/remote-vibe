# Remote Vibe 0.5.27 · 上线就绪状态

**可以公开测试；还不能宣布已经正式上架。** 0.5.27 将遥控、会话、画面和语音输入栏统一到 Material 3 设置页的视觉体系，导航采用 Compose，独立设置按钮保留；Android API/窗口/大字体/语言的本次验证范围见 verification.json。已补充 ARM64 / Android 16 的实际 16 KB 系统运行验证，关闭兼容回退后加载图形库并完成 UI 场景；厂商实体机仍待验证。0.5.24 完整代码审查确认并修复本机服务代理泄露、剪贴板恢复、迟到的语音输入、通知钩子覆盖、旧确认框串到新 Mac、消息恢复重复、回执写入故障、超长日志阻塞及官网依赖漏洞。具体触发条件、证据和验证范围见 CODE-REVIEW.md。配对撤销、终端授权选择与签名公证构建流程已在上一版提供。Android AAB、直装 APK、Mac DMG 和版本一致的验证材料一并提供。
