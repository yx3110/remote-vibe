# Remote Vibe 0.5.24 · 上线就绪状态

**可以公开测试；还不能宣布已经正式上架。** 本轮完整代码审查确认并修复本机服务代理泄露、剪贴板恢复、迟到的语音输入、通知钩子覆盖、旧确认框串到新 Mac、消息恢复重复、回执写入故障、超长日志阻塞及官网依赖漏洞。具体触发条件、证据和验证范围见 CODE-REVIEW.md。配对撤销、终端授权选择与签名公证构建流程已在上一版提供。Android AAB、直装 APK、Mac DMG 和版本一致的验证材料一并提供。
